...
from _compat import (
    USE_TCP, TMPDIR, connect_to_daemon, daemon_check_alive, cleanup_files
)
...
def _paths(name):
    n = name or NAME
    log = str(TMPDIR / f"bu-{n}.log")
    pid = str(TMPDIR / f"bu-{n}.pid")
    return log, pid


def _log_tail(name):
    p = str(TMPDIR / f"bu-{name or NAME}.log")
    try:
...
def daemon_alive(name=None):
    return daemon_check_alive(name or NAME)
        ...
        if system == "Windows":
            ...
            for p in paths:
                if os.path.exists(p):
                    ...
                    return
            subprocess.Popen(["cmd.exe", "/c", "start", "chrome"] + flags, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        elif system == "Darwin":
            subprocess.Popen(["open", "-n", "-a", "Google Chrome", "--args"] + flags, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
            subprocess.Popen(["google-chrome"] + flags, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        pass


def _wait_for_chrome_port(port: int = 9222, timeout: float = 15.0):
    """Poll localhost:port until Chrome's CDP endpoint responds or timeout expires.

    Returns immediately once the port is open. Silent on timeout — the caller's
    ensure_daemon() loop will surface the error with a proper message if needed.
    """
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
            with socket.create_connection(("127.0.0.1", port), timeout=1.0):
                return  # Port is open — Chrome is ready
        except OSError:
            time.sleep(0.25)
...
def ensure_daemon(wait=60.0, name=None, env=None):
    ...
    if daemon_alive(name):
        ...
        try:
            s = connect_to_daemon(name or NAME, timeout=3.0)
    ...
    if local and not _chrome_running():
        print("Chrome not running, auto-launching...")
        _launch_chrome()
        # Poll until Chrome's CDP port responds (or timeout after 15s).
        # This is faster on quick machines and safer on slow ones vs. a
        # fixed sleep that can both overshoot and undershoot.
        _wait_for_chrome_port(port=9222, timeout=15.0)

    for attempt in (0, 1):
        e = {**os.environ, **({"BU_NAME": name} if name else {}), **(env or {})}
        # On Windows, use 'python' directly; on Unix, use 'uv run'
        if USE_TCP:
            p = subprocess.Popen(
        ...
        if local and attempt == 0 and _needs_chrome_remote_debugging_prompt(msg):
            ...
            restart_daemon(name)
            continue
        log_path = str(TMPDIR / f"bu-{name or NAME}.log")
...
def restart_daemon(name=None):
    ...
    try:
        s = connect_to_daemon(n, timeout=5.0)
