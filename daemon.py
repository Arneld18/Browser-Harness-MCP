...
from _compat import USE_TCP, TMPDIR, get_port, write_port, connect_to_daemon, daemon_check_alive

NAME = os.environ.get("BU_NAME", "default")
SOCK = str(TMPDIR / f"bu-{NAME}.sock")  # Only used on Unix
...
async def serve(d):
    ...
    if USE_TCP:
        ...
        write_port(NAME, actual_port)
        log(f"listening on 127.0.0.1:{actual_port} (name={NAME}, remote={REMOTE_ID or 'local'})")
    else:
...
def already_running():
    return daemon_check_alive(NAME)


if __name__ == "__main__":
    if already_running():
        print(f"daemon already running for '{NAME}'", file=sys.stderr)
        sys.exit(0)
    # Truncate log and write PID using context managers to avoid handle leaks.
    Path(LOG).write_text("")
