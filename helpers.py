...
from _compat import connect_to_daemon, TMPDIR

NAME = os.environ.get("BU_NAME", "default")
INTERNAL = ("chrome://", "chrome-untrusted://", "devtools://", "chrome-extension://", "about:")


_daemon_verified = False

def _send(req):
    global _daemon_verified
    if not _daemon_verified:
        from admin import ensure_daemon
        try:
            ensure_daemon(wait=10.0)
            _daemon_verified = True
        except Exception as e:
            print(f"Warning: ensure_daemon failed: {e}")
            
    try:
        s = connect_to_daemon(NAME)
    except Exception:
        if _daemon_verified:
            _daemon_verified = False
            return _send(req)  # Retry once if daemon died
        raise RuntimeError("Daemon is unreachable. Run 'uv run python run.py --doctor' to check status.")
    s.settimeout(30)  # Allow up to 30s for CDP operations
...
def click_at_xy(x, y, button="left", clicks=1):
    if os.environ.get("BH_DEBUG_CLICKS"):
        ...
        try:
            ...
            path = capture_screenshot(str(TMPDIR / f"debug_click_{_debug_click_counter}.png"))
            img = Image.open(path)
